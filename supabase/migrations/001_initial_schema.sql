...
-- This is idempotent — safe to re-run on a blank database.
...
-- ─── Accounts ────────────────────────────────────────────
create type public.account_type as enum ('checking', 'savings', 'credit_card', 'shared', 'other');
...
-- ─── Transactions ────────────────────────────────────────
create type public.reimbursement_status as enum ('none', 'pending', 'partial', 'full');
create type public.transaction_type as enum ('expense', 'income', 'transfer', 'cc_payment', 'partner_transfer');
...
-- RLS: Transactions are scoped via their account's owner
...
-- ─── Settlement Groups ───────────────────────────────────
create type public.settlement_direction as enum ('to_shared', 'from_shared');
...
-- Security: caller must be seeding their own account (auth.uid() check)
